...
def main():
  ...
  colorama.init()
  path, output, repeat = handleInput()
    ...
    if(flag_timer):
      ...
      while(now - start < repeat):
        if(int(now-start) % 1 == 0):
          ...
          now = time.time()
          current_iteration += 1
          analyzeTxt(path, output)
    else:
      analyzeTxt(path, output)

def analyzeTxt(path, output):
...
def handleInput():
  global flag_show_gui, flag_screenshot, flag_show_image, flag_show_text, flag_timer
  if(len(sys.argv)==1):
    ...
    if(len(sys.argv)%2!=1):
      ...
      for i in range(1, len(sys.argv), 2):
        ...
        if(option_i == "-txt"): # show image
          if(argument_i == "y"):
            flag_show_text = True
        if(option_i == "-g"): # gui
          if(argument_i == "n"):
            flag_show_gui = False
  return name, output, repeat

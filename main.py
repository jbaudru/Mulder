    ...
    if(flag_timer):
      ...
      while(now - start < repeat):
        if(int(now-start) % 1 == 0):
          ...
          if(flag_show_gui):
            print(" New screenshot :", current_iteration, " - ", now-start, "sec.")
          now = time.time()
          current_iteration += 1
...
def handleInput():
  global flag_show_gui, flag_screenshot, flag_show_image, flag_show_text, flag_timer, flag_word
  if(len(sys.argv)==1):
...
def showResults(txt, blob, nbwords):
  ...
  if(flag_word):
    ...
    most_commo = counter.most_common(5)
  print(" =================================================")
  print(" Statistics : ")
